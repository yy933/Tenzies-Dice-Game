...
//#region node_modules/tween-functions/index.js
var require_tween_functions = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var tweenFunctions = {
...
//#endregion
//#region node_modules/react-confetti/dist/react-confetti.mjs
var import_jsx_runtime = require_jsx_runtime();
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_tween_functions = /* @__PURE__ */ __toESM(require_tween_functions(), 1);
...
var Particle = class {
	constructor(context, getOptions, x, y) {
		...
		this.angularSpin = randomRange(-.2, .2);
	...
	update(elapsed) {
		...
		if (this.rotateY >= 1 && this.rotationDirection === RotationDirection.Positive) this.rotationDirection = RotationDirection.Negative;
		else if (this.rotateY <= -1 && this.rotationDirection === RotationDirection.Negative) this.rotationDirection = RotationDirection.Positive;
		const rotateDelta = .1 * this.rotationDirection * frameTimeMultiplier;
		...
		if (drawShape && typeof drawShape === "function") drawShape.call(this, this.context);
	...
	constructor(canvas, getOptions) {
		...
		this.animate = (elapsed) => {
			...
			if (activeCount < numberOfPieces) {
				...
				for (let i = 0; i < numToAdd; i++) this.particles.push(this.getParticle());
			...
			for (let i = this.particles.length - 1; i >= 0; i--) {
				...
				p.update(elapsed);
				if (p.y > canvas.height || p.y < -100 || p.x > canvas.width + 100 || p.x < -100) if (recycle && activeCount <= numberOfPieces) this.particles[i] = this.getParticle();
				else this.removeParticleAt(i);
			}
...
var confettiDefaults = {
	...
	opacity: 1,
	debug: false,
	tweenFunction: import_tween_functions.easeInOutQuad,
	tweenDuration: 5e3,
	recycle: true,
	run: true
};
var Confetti = class {
	constructor(canvas, opts) {
		...
		this.setOptionsWithDefaults = (opts) => {
			const computedConfettiDefaults = { confettiSource: {
				x: 0,
				y: 0,
				w: this.canvas.width,
				h: 0
		...
		this.update = (timestamp = 0) => {
			...
			if (this.generator.animate(elapsed)) this.rafId = requestAnimationFrame(this.update);
			else {
				if (onConfettiComplete && typeof onConfettiComplete === "function" && this.generator.particlesGenerated > 0) onConfettiComplete.call(this, this);
	...
	set options(opts) {
		...
		if (this.generator) {
			...
			if (typeof opts.recycle === "boolean" && opts.recycle && lastRecycleState === false) this.generator.lastNumberOfPieces = this.generator.particles.length;
		}
		if (typeof opts.run === "boolean" && opts.run && lastRunState === false) this.update();
	}
};
var ref = import_react.createRef();
var ReactConfettiInternal = class extends import_react.Component {
	constructor(props) {
		super(props);
		this.canvas = import_react.createRef();
	...
	componentDidUpdate() {
		...
		if (this.confetti) this.confetti.options = confettiOptions;
	}
...
function extractCanvasProps(props) {
	...
	for (const prop in props) {
		...
		if (confettiOptionKeys.includes(prop)) confettiOptions[prop] = val;
		else if (refProps.includes(prop)) refProps[prop] = val;
		else rest[prop] = val;
	}
	return [
		confettiOptions,
...
var ReactConfetti = import_react.forwardRef((props, ref) => (0, import_jsx_runtime.jsx)(ReactConfettiInternal, {
	canvasRef: ref,
	...props
}));
//#endregion
